...
if st.button("5. Fit Global Best Model"):
    ...
    if comparison is not None:
        ...
        if best["type"] == "ARIMA":
            ...
            sigma2 = None
            try:
                if "sigma2" in fitted.param_names:
                    sigma2 = float(fitted.params[fitted.param_names.index("sigma2")])
            except Exception:
                pass

            if sigma2 is None or sigma2 <= 0:
                try:
                    sigma2 = float(np.var(fitted.resid))
                except Exception:
                    sigma2 = 1.0

            st.session_state["axis3_sigma2"] = sigma2
            ...
            # Send SARIMA parameters to spectral comparison exactly
            # as done in the ARIMA branch.
            if hasattr(fitted, "arparams"):
                st.session_state["axis3_phi"] = fitted.arparams
            else:
                st.session_state["axis3_phi"] = []
